...
<?php 
    ...
    if(!empty($_POST)){
        ...
        if(move_uploaded_file($_FILES['thesis_file']['tmp_name'], $targetfolder)){
            $sql = 'UPDATE thesis_table SET thesis_name = "'.$thesis_name.'", thesis_year = "'.$thesis_year.'", course = "'.$course.'", thesis_pdf_file = "'.$_FILES['thesis_file']['name'].'" WHERE id = '.$_GET['id'];
        }
        else{
            $sql = 'UPDATE thesis_table SET thesis_name = "'.$thesis_name.'", thesis_year = "'.$thesis_year.'", course = "'.$course.'" WHERE id = '.$_GET['id'];
        }
    ...
    $course = !empty($result[0]['course']) ? (int)$result[0]['course'] : '';
                                                ...
                                                <select name='course' class="form-control" id="selectCourse" branchid="<?php echo $_SESSION['branchid']; ?>" value="<?php echo $_SESSION['roomname']; ?>" style="padding: 0px 5px 0px 5px !important;"> 
                                                    ...
                                                    <?php
                                                        foreach ($branch as $row) {
                                                            $selected = $row['id'] == $course ? 'selected="selected"' : '';
